...
top_mcg <- dat %>% 
  ...
  select(MCG, Severity) %>%
  mutate(MCG = forcats::fct_inorder(as.character(MCG)))# at that way when check the levels of MCG, they appear in order of appearence ad not in order of increasing
top_mcg
...
set.seed(2017-06-29)# to make reproducible the random numbers
...
top_mcg %>% 
  ...
  knitr::kable(digits = 2)# very important  digits to round up in case how many dgits per column
```
...
region_dat <- dat %>%
  group_by(Region) %>%
  filter(n() > 5) %>% #taking away those less than 5 observations
  mutate(mean_sev = mean(Severity)) %>%
  mutate(Source = ifelse(Source == "wmn", "wmn", "producer field")) %>%
  arrange(desc(mean_sev)) %>%
  ungroup() %>%
  mutate(Region = forcats::fct_inorder(Region)) %>% # at that way when check the levels of MCG, they appear in order of appearence ad not in order of increasing
